...
from .utils import RowValidationError
...
def apply_row_standardisation(cleaned):
    # 1. Force columns to string (Use str() to ensure Python string objects)
    ...
    for col in must_be_strings:
        val = cleaned.get(col)
        if val is not None:
            cleaned[col] = str(val).lower()

    # Payment Status
    pay_status = cleaned.get("Payment Status")
    if isinstance(pay_status, str) and pay_status.lower().startswith("pend"):
        cleaned["Payment Status"] = "Pending"
    
    # Subject
    sub = cleaned.get("Subject")
    if isinstance(sub, str) and sub.lower().startswith("maths"):
        cleaned["Subject"] = "Mathematics"
    
    # Attendance Status
    status = cleaned.get("Attendance Status")
    if isinstance(status, str) and status.lower().endswith("mc"):
        cleaned["Attendance Status"] = "Absent-MC"

    # Duration (Hours)
    dur = cleaned.get("Duration (Hours)")
    if dur in [None, ""] or (isinstance(dur, float) and pd.isna(dur)):
        cleaned["Duration (Hours)"] = 1.0
        
    # Fees Charged
    fees = cleaned.get("Fees Charged")
    if isinstance(fees, str) and re.match(r'^[A-Za-z]+$', fees):
        cleaned["Fees Charged"] = 0
    elif fees is not None:
        cleaned["Fees Charged"] = parse_and_round_decimal(fees)
...
def parse_single_date(raw_val):
    ...
    # 2. Pre-clean the value once before the loop
    expected_formats = [
        "%Y-%m-%d",     # 2025-03-12
        "%m/%d/%Y",     # 01/05/2025
        "%d-%m-%Y",     # 25-04-2025
    ...
    # 4. Return original value and False (failure)
...
def parse_and_round_decimal(val, places=4):
    ...
    """
    # 1. Handle Null/Empty/NaN
    if pd.isna(val) or str(val).lower() in ["nan", "none", ""]:
        return None
    
    try:
        # 2. Extract number: Remove commas and use regex to find digits/dots
        # This replaces your previous .str.extract(r"([-+]?[\d.,]+)")
        ...
        # 3. Perform rounding using your existing logic
...
def validate_enums(cleaned_data, row_index):
    # Mapping of column names to Enum classes
    enum_map = {
    ...
    for col, enum_class in enum_map.items():
        if col in cleaned_data and cleaned_data[col] is not None:
            val = str(cleaned_data[col]).strip().lower()
            ...
            if val not in allowed_values:
                raise RowValidationError(
                    "INVALID_ENUM",
                    f"Row {row_index}: '{val}' is not a valid option for '{col}'. "
...
def validate_and_clean_row_dict(row_dict, file_category, stored_id, stored_content, row_index):
    # 1. CLEANING (Your efficient one-liner)
    cleaned = {k: (re.sub(r'\s+', ' ', v).strip() if isinstance(v, str) else v) for k, v in row_dict.items()}
    
    # 2. NULL RECORD CHECK
    is_null_row = all(v is None or v == "" or pd.isna(v) for v in cleaned.values())
    if is_null_row:
        raise RowValidationError("NULL_RECORD", f"Row {row_index} is completely empty.")

    cleaned = apply_row_standardisation(cleaned)

    # 3. TITLE CASE FORMATTING (Applied specifically to the fields that need it)
    title_cols = ["Student Name", "Tutor Name", "Level", "Payment Status", "Attendance Status", "Subject", "Status"]
    for col in title_cols:
        if col in cleaned and isinstance(cleaned[col], str):
            cleaned[col] = cleaned[col].title()

    capitalize_cols = ["Payment Status", "Subject", "Status"]
    for col in capitalize_cols:
        if col in cleaned and isinstance(cleaned[col], str):
            cleaned[col] = cleaned[col].capitalize()
    
    uppercase_cols = ["Invoice ID", "Assignment ID", "Log ID"]
    for col in uppercase_cols:
        if col in cleaned and isinstance(cleaned[col], str):
            cleaned[col] = cleaned[col].upper()

    # 2. ROW-BY-ROW DATE PARSING
    date_cols = ["Invoice Date", "Payment Date", "Session Date", "Start Date"]

    # 2. Iterate through each column to validate
    for col in date_cols:
        ...
        if col in cleaned and cleaned[col] not in [None, ""]:
            ...
            if not success:
                raise RowValidationError(
                    code="INVALID_DATE", 
                    message=f"Invalid date in row {row_index}, column '{col}': '{raw_val}'. Ensure format is YYYY-MM-DD."
                )

            else:
                cleaned[col] = val

    # 3. ROW-BY-ROW DECIMAL PARSING
    decimal_map = {
        "Amount": 4, 
        "Fees Charged": 4, 
        "Duration (Hours)": 1, 
    ...
    for col, places in decimal_map.items():
        if col in cleaned:
            ...
            cleaned[col] = parse_and_round_decimal(cleaned[col], places)
    # 4. DUPLICATE CHECKING (Using your tuple signature)
    criteria_map = {
    ...
    cleaned = validate_enums(cleaned, row_index)

    config = criteria_map.get(file_category, {})
    pk_key = config.get('pk')
    pk_val = str(cleaned.get(pk_key, '')).strip().upper()
    ...
    # DEBUG: Check if these exist in the sets
    # print(f"DEBUG Row {row_index} | ID '{id_signature}' in stored_id: {id_signature in stored_id}")
    # print(f"DEBUG Row {row_index} | Content {content_signature} in stored_content: {content_signature in stored_content}")
    ...
    if content_signature in stored_content:
        ...
        raise RowValidationError(
            ...
            message=f"Entry with identical details already exists at row {original_row}."
        )
    ...
    # print(f"DEBUG: Processing Row {row_index} | PK Key: {pk_key} | PK Value: '{pk_val}'")
    
    # Place this OUTSIDE your function
    FIELD_CONFIG = {
        "INVOICE": {
    ...
    if missing_fields:
        raise RowValidationError(
            code="NULL_FIELD", 
            message=f"Row {row_index} is missing mandatory data in: {', '.join(missing_fields)}."
        )

    
    return cleaned, file_category
